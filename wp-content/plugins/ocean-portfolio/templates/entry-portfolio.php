					...
					if ( 'on' == $overlay_icons ) { ?>
						<ul class="portfolio-overlay-icons">
							...
							if ( 'on' == $lightbox_icon
								...
								<li>
									<a href="<?php echo esc_url( wp_get_attachment_url( $img_id ) ); ?>" class="portfolio-lightbox no-lightbox" data-size="<?php echo esc_attr( $image[1] ); ?>x<?php echo esc_attr( $image[2] ); ?>" itemprop="contentUrl"><?php op_svg_icon( 'search' ); ?></a>
								</li>
							<?php } ?>
						</ul>
					<?php }
