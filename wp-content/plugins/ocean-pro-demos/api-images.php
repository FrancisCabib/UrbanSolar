...
class Demo_Api_Images {
	...
	private function constants() {
		...
		if ( ! defined( 'DEMO_API_VERSION' ) ) {
			define( 'DEMO_API_VERSION', 2 );
		}
	...
	public function demo_api_images_wp_media_scripts() {
		...
		if ( is_object($current_screen) && $current_screen->id != 'media_page_ocean-images' && self::demo_api_images_set_has_access() ) {
			wp_enqueue_style(
				'admin-demo',
			...
			if ( !self::activeProEcommerce() && self::activeOneFromProviders() ) {
				wp_enqueue_script(
					'demo-media',
	...
	public static function activeProEcommerce() {
		$oec_enable_ocean_images = get_option( 'oec_enable_ocean_images', 'no' );
		return $oec_enable_ocean_images == 'yes';
	}
}
